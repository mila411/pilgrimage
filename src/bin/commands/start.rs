...
fn validate_args(matches: &ArgMatches) -> CliResult<(String, usize, usize, String)> {
    let id = matches.value_of("id").ok_or_else(|| CliError::ParseError {
        ...
        message: "Broker ID is not specified. Please use the --id option."
            .to_string(),
    })?;
    ...
    let partitions: usize = matches
        ...
        .ok_or_else(|| CliError::ParseError {
            ...
            message:
                "Number of partitions not specified. Please use the --partitions option."
                    .to_string(),
        })?
        .parse()
    ...
    let replication: usize = matches
        ...
        .ok_or_else(|| CliError::ParseError {
            ...
            message: "Replication factor not specified. Please use the --replication option.".to_string(),
        })?
        .parse()
        .map_err(|e| CliError::ParseError {
            field: "replication".to_string(),
            message: format!("Invalid replication factor: {} (Please specify a positive integer)", e),
        })?;
    ...
    let storage = matches
        ...
        .ok_or_else(|| CliError::ParseError {
            ...
            message: "Storage path not specified. Please use the --storage option."
                .to_string(),
        })?;
